...
SimpleProfilesCreation <- function(files_path = Path_simplifiedProfilesCSV, poll, start_year, end_year) {
  ...
  for (year in start_year:end_year) {
    ...
    # Get sectors from monthly profile
    all_sectors <- monthly_profile$GNFR
    
    # Process F1, F2, F3 into F and keep standard GNFR sectors A-L
    target_sectors <- c("A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L")
    
    # Create the matrix for daily profiles with 12 target sectors
    sector_profile <- array(0, dim = c(get_days_in_year(year), length(target_sectors)))
    
    # Process each target sector
    for (sector_idx in seq_along(target_sectors)) {
      target_sector <- target_sectors[sector_idx]
      print(paste("Processing sector:", target_sector))
      
      if (target_sector == "F") {
        # Special case: F is the average of F1, F2, F3
        f1_data <- monthly_profile[monthly_profile$GNFR == "F1", ][, 4:15]
        f2_data <- monthly_profile[monthly_profile$GNFR == "F2", ][, 4:15]
        f3_data <- monthly_profile[monthly_profile$GNFR == "F3", ][, 4:15]
        
        f1_weekly <- weekly_profile[weekly_profile$GNFR == "F1", ][, 4:10]
        f2_weekly <- weekly_profile[weekly_profile$GNFR == "F2", ][, 4:10]
        f3_weekly <- weekly_profile[weekly_profile$GNFR == "F3", ][, 4:10]
        
        # Calculate average monthly and weekly factors
        if (nrow(f1_data) > 0 && nrow(f2_data) > 0 && nrow(f3_data) > 0) {
          monthlyForSector <- (f1_data + f2_data + f3_data) / 3
          weeklyForSector <- (f1_weekly + f2_weekly + f3_weekly) / 3
        } else {
          # If F1, F2, F3 not available, skip this sector
          cat("Warning: F1, F2, F3 not found for pollutant", poll, "- skipping sector F\n")
          sector_profile[, sector_idx] <- rep(1, get_days_in_year(year))  # Use neutral values
          next
        }
      } else {
        # Standard sector processing
        sector_match <- all_sectors[all_sectors == target_sector]
        if (length(sector_match) == 0) {
          cat("Warning: Sector", target_sector, "not found for pollutant", poll, "- using neutral values\n")
          sector_profile[, sector_idx] <- rep(1, get_days_in_year(year))  # Use neutral values
          next
        }
        
        monthlyForSector <- monthly_profile[monthly_profile$GNFR == target_sector, ][, 4:15]
        weeklyForSector <- weekly_profile[weekly_profile$GNFR == target_sector, ][, 4:10]
      }
      
      # Calculate daily profile for current sector
      daily_profile <- c()
      temp_weekday <- current_weekday  # Use temporary variable to not affect main loop
      
      for (month in 1:12) {
        week_factor <- 7 / days_in_month[month]
        
        for (day in 1:days_in_month[month]) {
          day_weight <- monthlyForSector[[month]] * weeklyForSector[[temp_weekday]]
          daily_profile <- c(daily_profile, day_weight)
          
          # Update temp_weekday, reset to 1 after Sunday (7)
          temp_weekday <- ifelse(temp_weekday == 7, 1, temp_weekday + 1)
        }
      }
    ...
    # Set dimension names to standard GNFR sectors A-L
    dimnames(sector_profile)[[2]] <- target_sectors
